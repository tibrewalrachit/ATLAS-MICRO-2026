...
def edge_inference(
    ...
    def inference(
        ...
        input_attention_qk: T.Tensor,
        weight_attention_qk: T.Tensor,
        output_attention_qk: T.Tensor,
        input_attention_sv: T.Tensor,
        weight_attention_sv: T.Tensor,
        output_attention_sv: T.Tensor,
        ...
        with T.CoreArray(shape=(core_num,), **core_array_kwargs):
            ...
            with T.SPMD(name="attention_qk", type="gemm", **attention_qk_partition_kwargs) as (core_M, core_K, core_N):
                with T.Kernel(T.ceildiv(core_M, min_tM), T.ceildiv(core_K, min_tK), T.ceildiv(core_N, min_tN), autotune=True) as (bM, bK, bN):
                    input_attention_qk = T.Tensor(shape=(core_M, core_K), strides=(core_K, 1), dtype=dtype)
                    weight_attention_qk = T.Tensor(shape=(core_K, core_N), strides=(1, core_K), dtype=dtype)
                    output_attention_qk = T.Tensor(shape=(core_M, core_N), strides=(core_N, 1), dtype=dtype)

                    attention_qk_input_tile = T.alloc(shape=(min_tM, min_tK), dtype=dtype)
                    attention_qk_weight_tile = T.alloc(shape=(min_tK, min_tN), dtype=dtype)
                    attention_qk_output_tile = T.alloc(shape=(min_tM, min_tN), dtype=dtype)
                    attention_qk_output_acc = T.alloc(shape=(min_tM, min_tN), dtype=dtype)

                    for m in T.Serial(bM):
                        for n in T.Serial(bN):
                            T.clear(attention_qk_output_tile)
                            for k in T.Serial(bK):
                                T.copy(input_attention_qk[m * min_tM, k * min_tK], attention_qk_input_tile)
                                T.copy(weight_attention_qk[k * min_tK, n * min_tN], attention_qk_weight_tile)
                                T.gemm(attention_qk_input_tile, attention_qk_weight_tile, attention_qk_output_acc)
                                T.add(attention_qk_output_acc, attention_qk_output_tile, attention_qk_output_tile) # Newly added element-wise vector ops
                            T.copy(attention_qk_output_tile, output_attention_qk[m * min_tM, n * min_tN])

            with T.SPMD(name="attention_sv", type="gemm", **attention_sv_partition_kwargs) as (core_M, core_K, core_N):
                with T.Kernel(T.ceildiv(core_M, min_tM), T.ceildiv(core_K, min_tK), T.ceildiv(core_N, min_tN), autotune=True) as (bM, bK, bN):
                    input_attention_sv = T.Tensor(shape=(core_M, core_K), strides=(core_K, 1), dtype=dtype)
                    weight_attention_sv = T.Tensor(shape=(core_K, core_N), strides=(1, core_K), dtype=dtype)
                    output_attention_sv = T.Tensor(shape=(core_M, core_N), strides=(core_N, 1), dtype=dtype)

                    attention_sv_input_tile = T.alloc(shape=(min_tM, min_tK), dtype=dtype)
                    attention_sv_weight_tile = T.alloc(shape=(min_tK, min_tN), dtype=dtype)
                    attention_sv_output_tile = T.alloc(shape=(min_tM, min_tN), dtype=dtype)
                    attention_sv_output_acc = T.alloc(shape=(min_tM, min_tN), dtype=dtype)

                    for m in T.Serial(bM):
                        for n in T.Serial(bN):
                            T.clear(attention_sv_output_tile)
                            for k in T.Serial(bK):
                                T.copy(input_attention_sv[m * min_tM, k * min_tK], attention_sv_input_tile)
                                T.copy(weight_attention_sv[k * min_tK, n * min_tN], attention_sv_weight_tile)
                                T.gemm(attention_sv_input_tile, attention_sv_weight_tile, attention_sv_output_acc)
                                T.add(attention_sv_output_acc, attention_sv_output_tile, attention_sv_output_tile) # Newly added element-wise vector ops
                            T.copy(attention_sv_output_tile, output_attention_sv[m * min_tM, n * min_tN])
