...
from frontend.ops.tilelang.cloud import cloud_inference, sanity_check
from frontend.ops.tilelang.edge import edge_inference
from frontend.ops.tilelang.general_mpmd import general_mpmd_autotune_inference, general_mpmd_fixed_inference
from frontend.ops.tilelang.general_spmd import general_spmd_autotune_inference, general_spmd_fixed_inference
...
def _build_cloud_ast_operator_inputs(
    ...
    # NOTE: After this step, all operators' shapes are already partitioned by TP/EP and batch size.
    set_cloud_shape(
        model_config=model_config,
...
def test_cloud_ast_inference(
    ...
    # Keep the signature aligned with `tests/inference_test_auto.py`; cloud TileLang inference
    # currently derives simulator element size from the kernel dtype path instead.
    del element_size
    ...
    if intermediate_result_dir == "":
        intermediate_result_dir = f"kick_the_tires/test_cloud_inference_tilelang/{model_config.name}_{core_num}cores"
...
def test_cloud_ast_all(
    ...
    for model_name, model_config_path, element_size, batch_size, context_length in test_config_list:
        ...
        output_dir = intermediate_result_dir or (
            f"kick_the_tires/test_cloud_inference_tilelang/"
...
def _build_edge_ast_operator_inputs(
    ...
    # NOTE: After this step, edge AST operators already carry batch/context information,
    # while hidden dimensions remain unsplit because the edge tiler explores mapping itself.
    set_edge_shape(
        model_config=model_config,
        attention_block=attention_block,
        ffn_moe_block=ffn_moe_block,
...
def test_edge_ast_inference(
    ...
    # Keep the signature aligned with `tests/inference_test_auto.py`; edge TileLang inference
    # currently derives simulator element size from the kernel dtype path instead.
    del element_size
    ...
    if intermediate_result_dir == "":
        intermediate_result_dir = f"kick_the_tires/test_edge_inference_tilelang/{model_config.name}_{channel_num}ch_{core_num}cores"
    os.makedirs(intermediate_result_dir, exist_ok=True)
...
def test_edge_ast_all(
    ...
    for model_name, model_config_path, element_size, batch_size, context_length in test_config_list:
        ...
        model_config = get_model_config_from_hf(model_name, model_config_path)
        output_dir = intermediate_result_dir or (
            f"kick_the_tires/test_edge_inference_tilelang/"
...
def test_general_tilelang_samples(
    system_config_path: str,
    intermediate_result_dir: str = "",
    gemm_tiling_cache_dir: str = "",
    num_workers: int = -1,
    ...
    worker_count = int(0.8 * multiprocessing.cpu_count()) if num_workers < 0 else num_workers
    output_root = intermediate_result_dir or "decaparated/test_general_tilelang"
...
if __name__ == "__main__":
    ...
    cloud_test_config_list = [
        ("llama3_70b", "configs/models/llama3_70b.json", 2, 8, 16 * 1024),
    ]
    ...
    edge_test_config_list = [
        ("llama3_8b", "configs/models/llama3_8b.json", 2, 8, 2 * 1024),
    ]
