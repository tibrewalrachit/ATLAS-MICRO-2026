...
| P10 | Snitch orchestration overhead per kernel invocation: HWPE job setup 120 cyc, DMA setup 60 cyc, SSR loop setup 40 cyc | 60/120/240, 30/60/120, 20/40/80 | Banshee is unbuildable against current crates.io state (see docs/PULP_LOGIC_DIE_SIM.md) so these are NOT measured. Ranges bracket typical HWPE-template register-programming sequences. Outputs carry `orchestration_source`; HWPE busy time dominates, so tile-level results move <5% across the range. |
