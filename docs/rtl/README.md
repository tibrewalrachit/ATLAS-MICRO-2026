...
Two of the design decisions here were prompted by the Kimi K3 ASIC write-up at
<https://www.luoluo.ai/blog/kimi-k3>, and it is worth saying which:

* **Sizing the accumulator to the value range.** That post sizes an INT4 MAC's
  accumulator from what the products can actually reach rather than from a
  round number. Applying the same reasoning to the MX block reduction is what
  produced §2.3 — and it is what closed the 1 GHz gap I had previously written
  off as needing another pipeline stage.
* **KDA**, the fixed-size recurrent attention state, and its published
  recurrence, which §4 implements.

The rest — the ATLAS architecture, HBDRAM, the Qwen3 MoE routing — comes from
this repository's own simulator and configuration files.
...
### 2.3 Sizing the accumulator to the format

The guard window `GUARD_W` sets how far below the largest product the
reduction stays exact. It is a parameter, and the right value is a property of
the *format*, not of the datapath:

| format | product exponent range | spread | guard needed for exactness |
|---|---|---:|---:|
| FP4 E2M1 | −6 … −2 | 4 | **4** |
| FP8 E4M3 | −18 … 10 | 28 | 28 |
| FP8 E5M2 | −34 … 24 | 58 | 58 |

An E2M1 block's products can only ever span four binary orders of magnitude,
so four guard bits make the reduction exact — there is nothing left to drop.
The default build carries 20, which is what an FP8-capable engine needs to keep
its error at 2⁻²², and that width propagates through the alignment field, the
carry-save tree and the final adder.

Building the engine for MXFP4 alone is therefore not a precision compromise,
it is removing width that provably carries no information:

| build | area (µm²) | cells | best slack at 1 ns | Fmax |
|---|---:|---:|---:|---:|
| `GUARD_W=20` (FP4 + FP8) | 2352 | 24527 | −0.484 ns | ~674 MHz |
| `GUARD_W=4` (MXFP4 only) | **1786** | **18088** | **+0.154 ns** | **~1181 MHz** |

24% smaller, and it **meets the 1 GHz ATLAS target with margin** — which the
wide build does not, and which I had previously assumed would need another
pipeline stage. It does not; it needs the accumulator sized to the data.

Both were checked against the *same* reference model, which computes at the
...
## 4. KDA: a fixed-size recurrent attention state

`rtl/attn/atlas_kda.sv` implements the Kimi Delta Attention recurrence:

```
S_t = diag(alpha_t) . S_{t-1}  -  beta_t k_t k_t^T S_{t-1}  +  beta_t k_t v_t^T
```

with `alpha_t` (decay) and `beta_t` (update) both from a 2-tap convolution
over `k_t`.

Written literally, `k k^T S` is d³ work per token. The last two terms share
the `k_t` outer product, so the engine computes
...
which is d². The factorisation is checked against the literal equation in
`tb/golden/atlas_kda.py` before either is used as a reference.

Note `diag(alpha)` multiplies from the left, so `alpha` scales **rows** of the
state — getting that backwards transposes the decay and is not visible in a
single-token test, which is why the testbench runs sequences.

### Why it matters here

The state is d×d whatever the context length. For a design whose premise is
bonded-DRAM bandwidth, and whose capacity is fixed by the DRAM dies it can
bond, that is a capacity question before it is a bandwidth one.
`pd/kv_footprint.py` works it against the real numbers — 94 layers, 4 KV
heads, 16 channels of 4 Gbit, tp=8:

| batch | context | conventional KV | KDA 3:1 |
|---:|---|---:|---:|
| 1 | 128K | 1.47 GB | 0.38 GB |
| 8 | 128K | **11.75 GB — over** | 3.02 GB |
| 32 | 32K | **11.75 GB — over** | 3.07 GB |

against 5.63 GB of headroom per chip once MXFP4 weights are placed. The ratio
is about 3.9×, but the ratio is not the point: a growing cache turns context
length into a capacity limit, and capacity on a bonded stack is not something
the controller or the NoC can trade against. Attention state stops moving with
context; 24 of 94 layers keep a conventional cache and the other 70 collapse to
17.5 MB that never grows.

The engine keeps a state save/restore port, because a recurrent state is what
replaces KV-cache paging and has to be swappable the same way.

Bugs the sequence test caught — all of them timing, none of them visible in a
single step:

* Operands were captured a cycle after the valid they travelled with, so the
  adder saw the next row's products and the next partial accumulator.
* `beta * k[i]` was produced by a multiplier issued alongside the one that
  consumed it, so the update silently used the *previous* row's gate. It is
  now computed for all rows at once, before the update pass.
