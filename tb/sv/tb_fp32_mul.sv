//===========================================================================
// tb_fp32_mul -- self-checking test of atlas_fp32_mul against binary64.
//
// Vectors come from tb/golden/gen_fmul_vectors.py and cover equal-exponent
// uniform random bit patterns.
//
//===========================================================================
`timescale 1ns/1ps
...
module tb_fp32_mul;
  ...
  initial begin
    ...
    if (!$value$plusargs("vectors=%s", fname)) fname = "tb/sv/fmul_vectors.txt";
    fd = $fopen(fname, "r");
    ...
    $fclose(fd);
    $display("tb_fp32_mul: loaded %0d vectors", nvec);
    ...
    if ((errors == 0) && (i_out == nvec))
      $display("TB_FP32_MUL: PASS  (%0d vectors, 0 mismatches)", nvec);
    else
      $display("TB_FP32_MUL: FAIL  (%0d vectors, %0d results, %0d mismatches)",
               nvec, i_out, errors);
    $finish;
  end
